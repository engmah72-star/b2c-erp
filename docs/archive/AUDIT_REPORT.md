...
| C4 — أرقام عملاء | ✅ UI 100% | كل ظهور phone في الـ UI محمي بـ maskPhone — backend subcollection deferred |
...
**UI Layer ✅ مكتمل 100%** — كل ظهور clientPhone محمي:
- approvals.html, production.html, archive.html, order-tracking.html (هذه الجلسة)
- design.html, design-workspace.html (سابقاً)
- print.html, shipping.html, shipping-dashboard.html, shipping-followup.html (محمية بـ ROLE_PAGES + canSeePhone)
- clients.html, reports.html (محمية بـ ROLE_PAGES)

**Backend layer مؤجل:** نقل `clientPhone` لـ subcollection `orders/{id}/contact/info` بقاعدة منفصلة على read.

**السبب:** Firestore لا تدعم field-level filtering — الحل الوحيد هو subcollection. يحتاج migration backfill + تعديل كل query في 60+ صفحة. Effort ~5-7 أيام.

**التخفيف الحالي:** الـ UI layer + RULE 8 client-side guards كافية في معظم السيناريوهات. التهديد المتبقي: مستخدم بصلاحية قراءة /orders يقدر يفتح DevTools ويقرأ الـ doc كاملاً.
