...
export function mountDateRangePicker({ container, storageKey, onChange }){
  ...
  if(!document.getElementById('drp-styles')){
    ...
    css.textContent = `
      ...
      .drp-label{font-size:var(--fs-base);font-weight:var(--fw-extra);color:var(--dim2,#647298);}
      .drp-chips{display:flex;gap:6px;flex-wrap:wrap;}
      .drp-chip{background:var(--bg3,rgba(255,255,255,.04));border:1px solid var(--line,rgba(255,255,255,.08));color:var(--snow,#e7eaf6);border-radius:20px;padding:6px 12px;font-size:var(--fs-sm);font-weight:var(--fw-bold);cursor:pointer;transition:all .15s;-webkit-tap-highlight-color:transparent;}
      ...
      .drp-current{margin-inline-start:auto;font-size:var(--fs-sm);font-weight:var(--fw-bold);color:var(--dim2,#647298);}
