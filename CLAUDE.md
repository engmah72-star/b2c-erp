...
## RULE PC1 — PROCESS-CENTRIC ARCHITECTURE (ميثاق مركزية العمليات — الفلسفة الأعلى)

> **النظام مبني حول العمليات والسياسات الثابتة، وليس حول المستخدمين.**
>
> الموظف، المورد، والعميل — مجرد **Actors** داخل **Workflow ثابت** يفرضه النظام.
> المستخدم **ينفّذ** العمليات فقط؛ أما القواعد فيفرضها النظام **تلقائياً**.
>
> هذه القاعدة **meta-charter فوق كل القواعد التقنية** — تُحدِّد *من يملك القرار*.
> الإجابة الدائمة: **القرار يملكه النظام (الكود المركزي)، لا المستخدم.**
> تعزّز وتوحّد: **C1 + A1 + V1 + P1 + X1.3 + X1.4 + H3 + L1**.

### PC1.1 — System Authority, Not User Authority
النظام لا يعتمد على وعي المستخدم أو خبرته التقنية، بل على **صرامة الـ Architecture والـ Workflow المركزي**.

| المبدأ | التطبيق |
|--------|---------|
| **القواعد في الكود** | كل rule تشغيلية/مالية/جودة تعيش في الـ central core، لا في إعداد يُعدّله مستخدم |
| **المستخدم Actor** | يُنفّذ action ضمن صلاحياته فقط — لا يُعرّف الـ action ولا شروطه |
| **النظام هو الحارس** | كل عملية تتحقق من الشروط → تمنع الأخطاء → تسجّل Audit → تسلّم المرحلة التالية بأمان |
| **استقرار رغم التغيير** | تغيّر الموظفين/الموردين لا يكسر التشغيل — القواعد ثابتة في الكود |

### PC1.2 — الثوابت التي **لا يملك أي مستخدم** تغييرها
القرارات التالية **مملوكة للنظام حصراً** (في الكود المركزي)، وممنوع تعريضها لأي مستخدم للتعديل — **بما فيهم الـ admin**:

| المجال الثابت | المالك المركزي (Single Owner) |
|---------------|-------------------------------|
| **قواعد التشغيل / منطق الـ Workflow** | `order-actions.js` (`orderActions.*`) + `orders.js` |
| **سياسات الانتقال بين المراحل** | `buildStageAdvance()` / `buildStageRevert()` / `validateStageRequirements()` / `advanceOrderStageWithLock()` في `orders.js` |
| **Validation Rules** | `validate*()` في `orders.js` + `financial-sync-engine.js` (V1) |
| **القواعد المالية** | `financial-sync-engine.js` فقط (FE + handlers + reversal) — RULE 2/4/G6 |
| **قواعد الاعتماد (Approvals)** | `approval-actions.js` + capability `approve_designs` |
| **قواعد الجودة** | الـ validators المركزية + stage requirements |
| **حدود الصلاحيات (من يُنفّذ ماذا)** | `core/permissions-matrix.js` (`canDo`/`canSee`/`hasPage`) |
| **قواعد التدقيق (Audit)** | `core/audit.js` (`auditEntry`) — H3 |

> **اختبار حاسم:** لو كان بإمكان مستخدم (عبر UI أو إعداد) أن يغيّر *منطق* أيٍّ مما سبق → **مخالفة PC1 تُرفض**. المنطق ثابت في الكود؛ يتغيّر فقط عبر PR مُراجَع (RULE G1/H1.8).

### PC1.3 — تمييز جوهري: تعديل **البيانات/البارامترات** ≠ تعديل **القواعد/المنطق**
النظام **يسمح** للأدوار المخوّلة بتهيئة **بيانات** ضمن قواعد ثابتة، **لكن لا يسمح** بتغيير القواعد نفسها:

| ✅ مسموح (Data/Parameters داخل قواعد ثابتة) | ❌ ممنوع (تغيير القاعدة/المنطق نفسه) |
|---|---|
| admin يضيف خامة في `master_lists/materials` | تغيير *كيف* تُحسب التكلفة أو التسعير في الكود |
| admin يضبط أسعار الشحن (`shipping_pricing`) | تغيير *منطق* تسوية الشحن أو الـ ledger |
| admin يمنح/يمنع capability لمستخدم (`users/{uid}.permissions`) | إضافة capability جديد أو تغيير *معنى* الصلاحية |
| تعديل سعر/ملاحظة على أوردر ضمن صلاحية `edit_orders` | تجاوز شرط انتقال مرحلة أو gating الدفع/الأرشفة |

الإعدادات (`settings.html`) **تهيئة بارامترات** لا **برمجة قواعد**. أي محاولة لجعل المنطق "configurable من الواجهة" = مخالفة PC1.

### PC1.4 — كل عملية = عقد ثابت (Operation Contract)
أي action في النظام **يجب** أن يفرض هذا التسلسل تلقائياً (تطبيق A1.3 + H1.5 + H3):
1. **يتحقق من الصلاحية** (`canDo` / STAGE_PERMISSIONS) — لا الاعتماد على إخفاء الزر فقط
2. **يتحقق من الشروط** (`validate*` → `{ ok, errors, warnings }`) — errors تمنع
3. **يمنع الأخطاء** ذرياً (writeBatch / transaction — RULE 3) — all-or-nothing
4. **يسجّل Audit Log** (`auditEntry` — H3: date + actor إلزامي)
5. **يسلّم المرحلة التالية بأمان** (timeline + stage transition مُمَركَز)
6. **يُرجع نتيجة موحَّدة** (`{ ok, errors, warnings, operationId, ... }` — H1.5)

النظام لا يثق في أن المستخدم "سيتذكّر" الشروط — **يفرضها في كل مسار تنفيذ**.

### PC1.5 — UI = View تنفّذ، لا View تُقرّر (تعزيز L1)
- الواجهة **تعرض** ما يسمح به الدور، و**تنادي** action مركزي واحد.
- الواجهة **لا** تحمل workflow logic، ولا validation، ولا financial rules، ولا permission logic.
- نتيجة: تغيير الموظف أو شكل الصفحة **لا يغيّر** سلوك العملية إطلاقاً.

### PC1.6 — منع Backdoors و Hidden Authority (تعزيز X1.3 + X1.4)
- ❌ ممنوع `if (uid === '...')` أو `if (email === '...')` لمنح سلطة استثنائية.
- ❌ ممنوع "god mode" يتجاوز الـ validators أو الـ FSE.
- ❌ ممنوع زر/مسار يتيح لمستخدم تخطّي شرط workflow أو financial gate.
- ✅ أي تجاوز مشروع (warning bypass) يمرّ عبر `bypassWarnings:true` المُعرَّف في الـ validator، مع audit — وليس عبر تعطيل القاعدة.

### PC1.7 — اختبار قبول أي feature/PR جديد (إلزامي)
| السؤال | الإجابة المطلوبة |
|--------|------------------|
| هل القاعدة (workflow/validation/مالية/اعتماد/جودة) تبقى في الكود المركزي؟ | ✅ نعم |
| هل المستخدم *ينفّذ* فقط (لا *يُعرّف* القاعدة)؟ | ✅ نعم |
| هل العملية تفرض: تحقق → منع خطأ → audit → تسليم آمن؟ | ✅ نعم |
| هل يستطيع مستخدم تغيير *منطق* أي قاعدة من الواجهة؟ | ❌ لا |
| هل هناك مسار يتجاوز الـ central actions / FSE / validators؟ | ❌ لا |

**أي إجابة خاطئة → الـ PR يُرفض ويُعاد تصميمه.**

### 🚫 القاعدة النهائية
**الهدف: منع الفوضى، تقليل الأخطاء البشرية، وضمان استقرار التشغيل حتى مع تغيّر الموظفين أو الموردين.**

النظام يُدار بـ **العمليات والسياسات الثابتة**، لا بـ **وعي المستخدم**.
أي drift عن PC1 (قاعدة هربت من الكود إلى يد المستخدم، أو مسار يتجاوز الـ core) = مخالفة خطيرة تُسجَّل في `GOVERNANCE_AUDIT.md` وتُعالَج فوراً (priority أعلى من باقي drifts، بمستوى E1).

---

## RULE PC2 — CENTRALIZED ROLE SYSTEM (ميثاق مركزية الأدوار)

> **الأدوار ليست مركز النظام — بل منفّذون (Executors) داخل Workflow مركزي ثابت.**
>
> النظام هو من يقود التشغيل، وليس المستخدمون.
> هذه القاعدة **تطبيق مباشر لـ PC1 على طبقة الأدوار**، وتعزّز **P1 + RULE 8 + A1 + C1.2**.

### PC2.1 — الدور = منفّذ، لا مالك قرار
كل Role في النظام يفعل ثلاثة أشياء فقط، بهذا الترتيب الثابت:
1. **يستلم Task** — يأتيه العمل من الـ Workflow المركزي (stage + assignment)، لا يخترعه
2. **ينفّذها** — ضمن صلاحياته (`canDo`) عبر central action واحد
3. **يسلّم المرحلة التالية** — عبر stage transition مُمَركَز يحدّد المستلم التالي

الدور **لا يملك** الـ Workflow — الـ Workflow يملك الدور ويوجّهه.

### PC2.2 — ما **لا يستطيعه أي Role** (مهما كان)
| ❌ ممنوع على أي دور | لماذا |
|---|---|
| **تغيير السياسات** | السياسات في الكود المركزي (PC1.2) — لا في يد الدور |
| **كسر الـ Workflow** | الانتقالات عبر `buildStageAdvance/Revert` + `advanceOrderStageWithLock` فقط |
| **تجاوز الـ Rules** | الـ validators تمنع early؛ لا مسار يتخطّاها |
| **منح نفسه صلاحية** | الـ capabilities تُمنح من admin عبر `users/{uid}.permissions` + audit — لا self-grant |
| **القفز فوق مرحلة** | تسلسل المراحل ثابت في `STAGES.next/prev` |

> هذا ينطبق على **كل** الأدوار الـ 8 — `admin` نفسه منفّذ ضمن النظام، لا فوقه (تعزيز X1.3: لا god mode).

### PC2.3 — الدور = Default Bundle من Capabilities (تعزيز P1.4)
- الـ Role **افتراض** لمجموعة `capabilities`، مُعرَّف مركزياً في `core/permissions-matrix.js`.
- **ممنوع** hardcoded role lists في الصفحات (`if role in [...]`) — استخدم `canDo(capability)`.
- ما يستطيع الدور تنفيذه = `capabilities` فقط؛ ما يراه = `canSee` fields؛ أين يدخل = `hasPage`.

### PC2.4 — التسليم بين الأدوار = Workflow Handoff لا قرار فردي
انتقال العمل من دور لدور (تصميم → طباعة → إنتاج → شحن) **يقوده النظام**:
- المرحلة الحالية + شروطها تحدّد **متى** يُسلَّم.
- الـ `nextAssigneeId/Name` يُمرَّر عبر الـ central action، لا يُكتب inline في الصفحة.
- كل تسليم = timeline entry عبر `auditEntry` (H3): من سلّم، متى، لمن.

### PC2.5 — اختبار قبول (لكل feature تمسّ الأدوار)
| السؤال | المطلوب |
|--------|---------|
| هل الدور يستلم/ينفّذ/يسلّم فقط؟ | ✅ نعم |
| هل صلاحياته من `permissions-matrix.js` (لا hardcoded)؟ | ✅ نعم |
| هل يستطيع تغيير سياسة/كسر workflow/تجاوز rule؟ | ❌ لا |
| هل التسليم يمرّ عبر central action + audit؟ | ✅ نعم |

### 🚫 القاعدة النهائية
**النظام يقود التشغيل؛ الأدوار تنفّذ داخله.** أي دور يقدر يغيّر سياسة أو يكسر workflow أو يتجاوز rule = مخالفة PC1+PC2 تُرفض وتُسجَّل في `GOVERNANCE_AUDIT.md`.

---

## RULE PC3 — CENTRALIZED ENTRY SYSTEM (ميثاق مركزية المدخل)

> **كل فئة داخل النظام لها مدخل واحد مركزي فقط — والنظام Workflow-driven، لا Page-driven.**
>
> أي موظف جديد يفتح **رابطاً واحداً**، فيعرف مباشرةً *ما المطلوب منه الآن*.
> هذه القاعدة **تطبيق PC1/PC2 على طبقة الدخول والتنقّل**، وتعزّز **N1 + W1 + C1.7/C1.8 + L1**، تحت سقف **E1** (ترحيل تدريجي، لا big-bang).

### PC3.1 — مدخل واحد لكل فئة (One Entry per Actor Type)
| الفئة | المدخل المركزي الوحيد | الواقع الحالي |
|------|----------------------|----------------|
| **الموظفون** | Employee Portal | `shell.html` (Runtime Shell، domains مُبوّبة بالصلاحيات) |
| **الإدارة** | Admin Control Center | `shell.html` → domain `admin` (يظهر لـ admin فقط عبر `domain-permissions.js`) |
| **العملاء** | Client Portal | `client-login.html` → `client-portal.html` |
| **الموردون** | Supplier Portal | `supplier-requests.html` |

> **ملاحظة معمارية:** الموظف والإدارة يدخلان من نفس الـ shell، لكن **ما يراه كلٌّ منهما يُحدَّد بالصلاحيات** (`getAllowedDomains` / `canSeeDomain`) — فهو منطقياً "Employee Portal" و"Admin Control Center" داخل runtime واحد، لا تطبيقان منفصلان. هذا يحقق المركزية بدون ازدواج كود.

### PC3.2 — كل فئة ترى **فقط** ما يخصّها
المدخل يعرض للفئة/الدور حصراً (تعزيز RULE 8 + P1.5):
- **الـ Tasks** الخاصة بها (المُسنَدة لها / لمرحلتها)
- **العمليات** المرتبطة بها فقط
- **الـ Alerts / Signals** المطلوبة (عبر `core/runtime-shell/signals.js`)
- **المرحلة الحالية** للأوردر (`order.stage` — W1.1)
- **الإجراءات المسموح بها** فقط (`canDo` — الزر يُخفى قبل الضغط، لا "error بعد الضغط")

**بدلاً من:** تعدّد الصفحات والواجهات العشوائية وتشتّت نقاط الدخول.

### PC3.3 — Workflow-driven, NOT Page-driven
- السؤال اليومي يتحول من *"أيّ صفحة أفتح؟"* إلى *"ما المهمة التالية؟"* (تعزيز `RUNTIME_OPERATING_MODEL`).
- الـ shell يقود التنقّل (rail + context sidebar)، والـ workspace ينفّذ، والـ signals توجّه الانتباه.
- الصفحات = **views لنفس الـ Order** (C1.8)، تُفتح داخل الـ workspace، لا وجهات منفصلة يتنقّل بينها المستخدم يدوياً.
- كل تنقّل تشغيلي عبر `navigatePage()` (N1.1)، لا `location.href` مبعثر.

### PC3.4 — حدود الدور داخل المدخل (تعزيز PC2.2)
كل Role يتحرك داخل الـ Workflow المركزي الثابت، ولا يملك:
- ❌ تغيير السياسات · ❌ تغيير الـ Workflow · ❌ تجاوز الـ Rules
- ❌ **الوصول العشوائي لباقي المراحل** — يرى ويعمل على مرحلته/مهامه فقط (gating عبر domains + `STAGE_PERMISSIONS` + `canSee`)

### PC3.5 — الأهداف (معايير النجاح)
مركزية التشغيل · تقليل الفوضى · سهولة الاستخدام والتدريب · تقليل الأخطاء · تسريع العمل · قابلية التوسّع.
**المحك النهائي:** موظف جديد يفتح **رابطاً واحداً** ويعرف فوراً ما المطلوب منه الآن — بلا خريطة صفحات في رأسه.

### PC3.6 — الترحيل تحت سقف E1 (إلزامي)
- ❌ ممنوع حذف/كسر أي صفحة legacy load-bearing لفرض المدخل الموحَّد دفعة واحدة.
- ✅ الـ god pages تبقى تعمل standalone و تُحمَّل داخل الـ shell كـ workspace iframe (N1.2) أثناء التحوّل.
- ✅ أي **تطوير دخول/تنقّل جديد** يُبنى على الـ shell + portals القائمة، خلف flag عند اللزوم (E1.8)، تدريجياً (G9).
- ✅ إضافة فئة دخول جديدة ممنوعة خارج الأطراف الأربعة (BUSINESS DNA SCOPE).

### 🚫 القاعدة النهائية
**مدخل واحد لكل فئة · Workflow-driven · يرى ما يخصّه فقط.** أي نقطة دخول جديدة موازية، أو تنقّل page-driven عشوائي، أو وصول دور لمراحل ليست له = مخالفة PC3 تُرفض وتُسجَّل في `GOVERNANCE_AUDIT.md` (تُعالَج تدريجياً حسب E1/G9).
...
# RULE N1 — RUNTIME SHELL NAVIGATION CONTRACT (Post-Cleanup Phase 2)

> **النظام في طور الانتقال من Legacy Multi-Page إلى Runtime Shell.**
> هذه القاعدة توثّق الـ contracts بين الـ shell والـ god pages لضمان
> الـ coexistence السليم خلال الـ migration.

## N1.1 — Shell-aware navigation
كل ضغطة على رابط تشغيلي **يجب** تمر عبر `window.navigatePage(url)` من
`core/shell-navigate.js`:

```html
<!-- ❌ ممنوع (full page reload حتى داخل الـ shell) -->
<button onclick="location.href='design.html?id=123'">Open</button>

<!-- ✅ المسموح (shell-aware — يفتح في الـ workspace iframe لو موجود) -->
<button onclick="navigatePage('design.html?id=123')">Open</button>
```

السلوك:
- لو الصفحة داخل `shell.html` iframe → `B2CShell.openInWorkspace(url)` (no reload)
- لو standalone → `window.location.href = url` (نفس السلوك القديم)

`window.navigatePage` متاح تلقائياً على كل صفحة بتحمّل `sidebar-config.js`.

## N1.2 — Embed-mode contract
لما الصفحة بتتحمّل بـ `?embed=1` في الـ URL، `sidebar-config.js` بيضيف
`html.embed-mode` class قبل الـ paint الأول.

`shared.css:296-307` بيخفي تلقائياً:
- `.sidenav` (الـ sidebar القديم)
- `.topbar` (الـ topbar القديم)
- `.mob-nav` (الموبايل nav السفلي)
- `.mob-menu-btn`
- `.fab` (الـ FAB القديم)
- `.nav-overlay`

ويضبط `.app-shell` / `.main` / `.content` لـ full-width embedded layout.

**Runtime Shell بيضيف `?embed=1` تلقائياً لكل iframe load** عبر
`core/runtime-shell/workspace-host.js`. الـ god pages ما تحتاجش تعمل
أي حاجة — فقط متعتمدش على وجود الـ topbar/sidenav داخل الـ shell.

## N1.3 — Hash routing reservation
الـ shell بيستخدم `location.hash` بـ prefix `#ctx=` لـ context navigation
(مستقبلاً — لـ entity deep-linking داخل الـ workspace).

**ممنوع** أي صفحة (god page أو غيرها) تستخدم `location.hash` لـ:
- ❌ Tab switching (`#overview`, `#designs`, ...)
- ❌ Filter state (`#filter=urgent`)
- ❌ أي page-internal state

**البديل:** استخدم query string:
```js
// ❌ ممنوع
...
الـ pages اللي حالياً بتستخدم hash (`my-profile.html`, `employee-profile.html`,
`reports.html`, `order.html` `#client=`) → في Phase 9 من
`CLEANUP_PLAN.md` للترحيل التدريجي.

## N1.4 — Stable Core (N1 addendum)
إضافة على H1.8 / H2.7 / H3.7:
- `core/shell-navigate.js` (الـ helper الموحَّد)
- `core/runtime-shell/*` (الـ shell modules — rail, context-sidebar, workspace-host, signals, fab)
- `core/domains/*/sidebar.js` (الـ 8 domain configs)
