...
(function() {
  ...
  function injectStyles() {
    ...
    s.textContent = ''
      ...
      + '.cp-input-icon{font-size:18px;opacity:.6}'
      ...
      + '.cp-item{display:flex;align-items:center;gap:10px;padding:9px 16px;cursor:pointer;color:var(--snow,#e8eaf0);font-size:13px;font-weight:600;transition:background .08s}'
      + '.cp-item:hover,.cp-item.cp-active{background:rgba(167,139,250,.18)}'
      + '.cp-item-ico{font-size:18px;width:24px;text-align:center;flex-shrink:0}'
      + '.cp-item-label{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}'
      + '.cp-item-hint{font-size:10px;color:var(--dim2,#5c6878);opacity:.7}'
      + '.cp-empty{padding:30px 16px;text-align:center;color:var(--dim2,#5c6878);font-size:13px}'
      ...
      + '.cp-kbd{display:inline-block;background:var(--bg3,#1e2535);border:1px solid var(--line,#2a3348);border-radius:4px;padding:1px 6px;font-family:monospace;font-size:10px;color:var(--snow,#e8eaf0)}'
      + '@media(max-width:600px){.cp-overlay{padding-top:40px;padding-inline:10px}.cp-modal{width:100%;max-height:80vh}.cp-footer{font-size:10px;gap:8px}}'
    ;
