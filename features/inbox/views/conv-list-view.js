  ...
  if (!items.length) {
    ...
    if (searchQuery) {
      ...
      html = `<div style="padding:28px 20px;text-align:center;color:var(--ws-text-dim);font-size:var(--fs-md);line-height:1.9">
        ...
        <button type="button" onclick="retryEnsureChannels()" style="margin-top:12px;padding:8px 16px;border-radius:var(--rad);border:1px solid var(--ws-border);background:var(--ws-bg-elev);color:var(--ws-text);cursor:pointer;font-family:inherit;font-size:var(--fs-base);font-weight:var(--fw-bold)">🔄 إعادة الإعداد</button>
        <button type="button" onclick="newDM()" style="margin-top:12px;margin-right:6px;padding:8px 16px;border-radius:var(--rad);border:none;background:#00a884;color:#fff;cursor:pointer;font-family:inherit;font-size:var(--fs-base);font-weight:var(--fw-bold)">✏️ محادثة جديدة</button>
